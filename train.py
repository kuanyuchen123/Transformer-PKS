...
def train(model, optimizer, data_loader):
    ...
    for epoch in range(params.n_epoch):
        for step, (src_X, src_Y, tgt_Y, src_K) in enumerate(data_loader):
            ...
            Y_hidden = encoder(src_Y, src_Y_pad_mask)
            K_hidden = encoder(src_K, src_K_pad_mask)
            ...
            kld_loss = KLDLoss(prior, posterior)
            loss = nll_loss + kld_loss + bow_loss
            loss.backward()
            optimizer.step()

            avg_loss['nll_loss'] += nll_loss
            avg_loss['kld_loss'] += kld_loss
            avg_loss['bow_loss'] += bow_loss
            if (step + 1) % 50 == 0:
                avg_loss['nll_loss'] /= 50
                avg_loss['kld_loss'] /= 50
                avg_loss['bow_loss'] /= 50
                print(
                    "Epoch [%.2d/%.2d] Step [%.4d/%.4d]: nll_loss=%.4f, kld_loss=%.5f, bow_loss=%.4f"
                    % (
                        epoch + 1,
                        params.n_epoch,
                        step + 1,
...
if __name__ == "__main__":
    ...
    model = [encoder, decoder, knowledge_manager]
